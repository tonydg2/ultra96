...
/* ADG Note: NUMBER_OF_BDS_PER_PKT will determine when/where the TLAST pulses on TX/MM2S, 
 with NUMBER_OF_PKTS_TO_TRANSFER=1 and NUMBER_OF_BDS_PER_PKT=2, two "frames" are transferred with
	...
	for (ChanId = 1; ChanId <= num_channels; ChanId++) {
		...
		for (j = 0 ; j < NUMBER_OF_PKTS_TO_TRANSFER; j++) {
			for (i = 0 ; i < NUMBER_OF_BDS_PER_PKT; i++) {
				...
				//memset((void *)RxBufferPtr, 0, MAX_PKT_LEN);                  // gets stuck here ADG. there's no interface to access the BRAM - only MCDMA can access it
	...
	for (ChanId = 1; ChanId <= num_channels; ChanId++) {
		...
		for (j = 0 ; j < NUMBER_OF_PKTS_TO_TRANSFER; j++) {
			for (i = 0 ; i < NUMBER_OF_BDS_PER_PKT; i++) {
				...
				/* Clear the receive buffer, so we can verify data */
				//memset((void *)TxBufferPtr, 0, MAX_PKT_LEN);  // adg this gets stuck. only MCDMA has BRAM access
            ...
            XMcDma_BdSetCtrl(TxBufferPtr, CrBits);
            // adg end */
		}
